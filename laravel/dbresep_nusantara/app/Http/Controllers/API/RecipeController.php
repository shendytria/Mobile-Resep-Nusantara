...
class RecipeController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Konversi dari string JSON ke array agar validasi bisa jalan
        if ($request->has('ingredients') && is_string($request->ingredients)) {
            $decoded = json_decode($request->ingredients, true);
            if (is_array($decoded)) {
        ...
        if ($request->has('steps') && is_string($request->steps)) {
            $decoded = json_decode($request->steps, true);
            if (is_array($decoded)) {
                $request->merge(['steps' => $decoded]);
            }
        }
        ...
        try {
            ...
            // Tangani unggah thumbnail_photo
            if ($request->hasFile('thumbnail_photo')) {
                Log::info('Thumbnail photo detected', ['filename' => $request->file('thumbnail_photo')->getClientOriginalName()]);
                if ($recipe->thumbnail_photo && Storage::disk('public')->exists($recipe->thumbnail_photo)) {
                    Storage::disk('public')->delete($recipe->thumbnail_photo);
                    Log::info('Old thumbnail deleted', ['path' => $recipe->thumbnail_photo]);
                }
                $path = $request->file('thumbnail_photo')->store('recipes/thumbnails', 'public');
                if ($path) {
                    $data['thumbnail_photo'] = $path;
                    Log::info('New thumbnail stored', ['path' => $path]);
                } else {
                    Log::error('Failed to store thumbnail');
                    return response()->json(['success' => false, 'message' => 'Gagal menyimpan thumbnail'], 500);
                }
            }
            ...
            return response()->json(['success' => false, 'message' => 'Gagal memperbarui resep: ' . $e->getMessage()], 500);
        }
    }
    ...
    {
        Log::info("Removing recipe $recipeId from collection $collectionId for user " . Auth::id());
        ...
        Log::info("Recipe $recipeId successfully removed from collection $collectionId");
