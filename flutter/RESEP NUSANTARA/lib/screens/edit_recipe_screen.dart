...
class _EditRecipeScreenState extends State<EditRecipeScreen> {
  ...
  Future<void> _fetchCategories() async {
    ...
    try {
      ...
      print(
        'EditRecipeScreen: Categories response: ${response.statusCode} - ${response.body}',
      );
      if (response.statusCode == 200) {
        ...
        print(
          'EditRecipeScreen: Unauthorized (categories), redirecting to login',
        );
  ...
  try {
    ...
    final isEditingThumbnail = _pickedXFile != null || _pickedImageFile != null || _webImage != null;

    if (!isEditingThumbnail) {
      // Tanpa thumbnail, kirim JSON biasa (PUT)
      final response = await http.put(
        ...
        body: jsonEncode({
          ...
          'steps': instructions.asMap().entries.map((entry) => {
      ...
      print('JSON PUT response: ${response.statusCode} - ${response.body}');
      ...
      // Dengan thumbnail, gunakan MultipartRequest
      final request = http.MultipartRequest(
        'POST', // WAJIB POST saat pakai Multipart (kita override via _method)
        Uri.parse('$baseUrl/api/recipes/${recipe.id}'),
      );
      ...
      request.fields['ingredients'] = jsonEncode(ingredients.map((i) => {
        'name': i['name'],
        'quantity': i['quantity'],
        'unit': i['unit'],
      }).toList());

      request.fields['steps'] = jsonEncode(instructions.asMap().entries.map((entry) => {
        'step_number': entry.key + 1,
        'description': entry.value,
      }).toList());
      ...
      if (_pickedXFile != null) {
        final extension = _pickedXFile!.name.split('.').last.toLowerCase();
        final mimeType = lookupMimeType(_pickedXFile!.name) ?? 'image/jpeg';
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final error = jsonDecode(response.body);
        throw Exception(error['message'] ?? 'Gagal update resep');
      }
    }
  ...
  InputDecoration _buildInputDecoration(String label, {IconData? icon}) {
    return InputDecoration(
      labelText: label,
      prefixIcon:
          icon != null ? Icon(icon, color: darkGreen.withOpacity(0.7)) : null,
                                ...
                                child:
                                    _pickedXFile != null
                                        ...
                                        ? ClipRRect(
                                          ...
                                          child: Image.network(
                                            '$baseUrl/storage/${_recipe!.imageUrl}',
                                            fit: BoxFit.cover,
                                            width: double.infinity,
                                            errorBuilder:
