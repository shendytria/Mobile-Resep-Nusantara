  ...
  Widget build(BuildContext context) {
    ...
    final String imageUrl = recipe.imageUrl != null && recipe.imageUrl!.isNotEmpty
        ? (recipe.imageUrl!.startsWith('http')
            ? recipe.imageUrl!
            : '$baseUrl/storage/${recipe.imageUrl!.replaceAll(RegExp(r'^/+'), '')}')
        : '';

    if (kDebugMode) {
      print('RecipeCard: Using image URL: $imageUrl');
    }
    ...
    Widget _buildFallbackImage() {
      return Image.asset(
        ...
        errorBuilder: (context, error, stackTrace) {
          if (kDebugMode) {
            print('RecipeCard: Error loading fallback asset: $error');
          }
          return Container(
                    ...
                    children: [
                      ...
                      if (recipe.isInCollection)
                        Container(
                          ...
                          child: const Icon(Icons.bookmark, color: Colors.yellow, size: 18),
                        ),
                    ],
                  ),
                ),
